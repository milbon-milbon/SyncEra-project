...
export const useGetSavedSummaryReport = (employeeId: string, createdAt: Date) => {
    ...
    const fetchSavedSummaryReport = async () => {
        try {
        const response = await fetch(`http://localhost:8000/client/print_saved_summary_report/${employeeId}/?created_at=${createdAt}`);

        if (!response.ok) {
            throw new Error(`Failed to fetch saved summary report: ${response.status} ${response.statusText}`);
        }
