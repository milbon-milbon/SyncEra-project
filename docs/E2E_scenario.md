...
1. **基本的なページタイトルの確認**
   - **テストケース**: `basic test`
   - **説明**: `http://localhost:3000/employee-list` にアクセスし、ブラウザタブのタイトルが「SyncEra」であることを確認する。

2. **ホームページの確認**
   - **テストケース**: `homepage test`
   - **説明**:
     - ホームページ (`http://localhost:3000`) にアクセス。
     - ページタイトルが「SyncEra」であることを確認。
     - 「管理画面」ボタンが表示されることを確認。
     - 「ログイン」および「今すぐ始める」ボタンが表示されることを確認。
     - ページ末尾に「© 2024 SyncEra. All rights reserved.」が表示されることを確認。
     
3. **ログインと社員一覧ページの確認**
   - **テストケース**: `should log in and verify employee list page`
   - **説明**:
     - ログインページ (`http://localhost:3000/login/employee`) にアクセスし、ログイン情報を入力。
     - 社員一覧ページ (`http://localhost:3000/employee-list`) に遷移することを確認。
     - 社員一覧ページで以下を確認:
       - 「社員一覧」の文字が表示されていること。
       - 社員の名前（「KU-MIN」「SAYOKO」「MIKIKO」「MEME」）が表示されていること。
       - サイドバーに「社員登録」「ホーム」が表示されていること。
       - 各社員のカードに「日報」「1on1アドバイス」「キャリアアンケート結果」のボタンが表示されていること。
       - MEMEの「日報を見る」ボタンをクリックし、「MEMEの日報」「社員情報」「最新の日報」が表示されることを確認。
       - 「日報サマリーを見る」ボタンをクリックし、日報サマリーページに遷移することを確認。
       - 「1on1アドバイスを見る」ボタンをクリックし、1on1アドバイスページに遷移することを確認。
       - 「キャリアアンケート結果を見る」ボタンをクリックし、キャリアアンケート結果ページに遷移することを確認。
