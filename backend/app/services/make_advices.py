...
def make_advices(slack_user_id: str, start_date: date, end_date: date):
    try:
        ...
        **小見出し** <\n>
        ...
        【理想的な出力例】
        **2024年8月21日までのdaily_reportおよびtimesの投稿をもとにしています。**
        ...
        **✔️部下が食いつく話題**
        - バックエンド開発の楽しさについて
        - 「最近のバックエンド開発でどんなことを楽しんでいるの？」
        - ユーザー体験についての意識
        - 「最近、ユーザーの操作性を意識しているようだけど、どう感じているの？」
        ...
        **✔️具体的な褒め方**
        - 「最近の関数実装に夢中になっている姿勢が素晴らしいね。特にユーザーの視認性を考慮しているのが良い！」
        - 「バックエンドの開発を楽しんでいる様子が伝わってきて、頼もしいサーバーを作る意欲を感じるよ。」
        ...
        **✔️具体的なフィードバック**
        - 「最近のキャリアアンケートでも、自分の幅を広げたいと答えていたね。具体的にどんなスキルを身につけたい？」
        ...
        **✔️日常の楽しみ**
        - 「最近のマックに行った時の感想は？何を食べたのが一番美味しかった？」

        **✔️リフレッシュ方法の確認**
        - 「仕事の合間にどんな方法でリフレッシュしているの？」
        ...
        **✔️距離を縮めるために**
        - 「日報を読んでいて、バックエンドの楽しさについて書いてあったのが印象的だったよ。」
        - 「timesの投稿でも、マックについて楽しそうに話していたね。食事でリフレッシュするのはいいアイデアだね。」

        ---

        このように、1on1は部下の本音を引き出し、信頼関係を築くための重要な機会です。リラックスした雰囲気で会話を進め、具体的なフィードバックを心がけましょう。
